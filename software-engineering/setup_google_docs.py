...
def create_document(docs_service, drive_service):
    ...
    add_normal(
        ...
        "with hybrid retrieval (combining semantic vector search and BM25 keyword matching "
        "in a unified PostgreSQL database using pgvector and ParadeDB) to "
    ...
    add_normal(
        ...
        "extraction, then performs hybrid retrieval (semantic vector search via pgvector + "
        "BM25 keyword search via ParadeDB) against the unified PostgreSQL database. Results "
        "are ranked by relevance, reranked using Cohere Rerank 4, and presented with "
    ...
    add_normal(
        ...
        "and stores everything\u2014metadata, embeddings, and full-text search indexes\u2014in a single "
        "PostgreSQL database (with pgvector for vector search and ParadeDB for BM25 keyword search) "
        "for future retrieval. Cross-referencing is maintained via unique document IDs."
    )
    ...
    spec_rows = [
        ...
        ("Main Success Scenario",
         ...
         "5. The system converts the query into a vector embedding using Qwen3-Embedding-0.6B.\n"
         "6. The system performs hybrid retrieval against the unified PostgreSQL database: "
         "BM25 keyword search (via ParadeDB) and semantic search (vector cosine similarity "
         "via pgvector) run in parallel.\n"
         "7. The system merges and re-ranks results using Reciprocal Rank Fusion, applies "
         "Cohere Rerank 4 for neural reranking, and incorporates metadata signals "
         "(recency, document type).\n"
    ...
    sections = [
        ...
        },
        {
            ...
            "desc": (
                ...
                "queries proceed through intent classification, embedding generation (using "
                "Qwen3-Embedding-0.6B), and parallel hybrid retrieval (BM25 keyword search via "
                "ParadeDB + semantic vector search via pgvector, both within a unified PostgreSQL "
                "database). Results are merged via Reciprocal Rank Fusion, reranked using Cohere "
                "Rerank 4, filtered by metadata and RBAC permissions, and displayed to the user. If "
        ...
        },
        {
            ...
            "intro": (
                ...
                "API, NLP engine, unified PostgreSQL database (pgvector + ParadeDB), reranker, and "
                "ranking service."
            ),
            ...
            "desc": (
                ...
                "intent and generate a vector embedding of the query using Qwen3-Embedding-0.6B. "
                "Two retrieval paths execute in parallel against the unified PostgreSQL database: "
                "pgvector performs semantic similarity search while ParadeDB performs BM25 keyword "
                "search. Both result sets are passed to the Ranking Service, which merges them using "
                "Reciprocal Rank Fusion and then applies Cohere Rerank 4 for neural reranking, "
                "followed by any metadata filters. The ranked results are returned to the frontend "
                "for display. The query is logged in PostgreSQL for analytics and audit purposes."
            ),
        },
        {
            ...
            "desc": (
                ...
                "Layer contains the Input Validator (enforcing query boundaries\u2014length limits, "
                "language, PII blocking, prompt injection detection), the NLP Engine (intent "
                "classification + embedding generation using the Qwen3-Embedding-0.6B model), the "
                "Reranker (Cohere Rerank 4 for neural reranking of candidate results), and the "
                "Ranking Service (Reciprocal Rank Fusion of keyword and semantic results followed by "
                "reranking). The Data Layer includes a unified PostgreSQL database (with pgvector for "
                "semantic search over document embeddings and ParadeDB for BM25 keyword search, "
                "structured metadata, and audit logs), and the Document Ingestion Pipeline (which uses "
                "Docling to parse incoming documents, extracts text and metadata, chunks content, "
                "generates embeddings via Qwen3-Embedding-0.6B, and stores everything in PostgreSQL "
                "with cross-referenced document IDs)."
            ),
        },
        {
            ...
            "desc": (
                ...
                "application with server-side rendering and routes API calls; (2) the Backend Container "
                "runs the FastAPI application on a Uvicorn ASGI server with the NLP Engine "
                "(Qwen3-Embedding-0.6B embedding model and intent classifier) loaded in-process "
                "for low-latency inference; (3) the Database Container runs PostgreSQL 16 with pgvector "
                "and ParadeDB extensions, providing unified storage for document metadata, vector "
                "embeddings (semantic search via pgvector), BM25 keyword search (via ParadeDB), and "
                "audit logs; (4) the Keycloak Container provides identity and access management, "
                "integrated with NextAuth.js on the frontend for authentication and RBAC. Communication "
                "between the frontend and backend uses REST API calls. The backend communicates with "
                "PostgreSQL via SQL/ORM."
            ),
        },
    ]
