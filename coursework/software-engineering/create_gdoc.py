...
def build_content(image_uris):
    ...
    body(
        ...
        "into the system. The ingestion pipeline uses Docling to parse documents and extract "
        "text and metadata, chunks long documents, generates vector embeddings using "
        "Qwen3-Embedding-0.6B, and stores both the metadata and embeddings in a unified "
        "PostgreSQL database (with pgvector for vectors) for future retrieval. "
        "Cross-referencing is maintained via unique document IDs."
    )
...
def main():
    ...
    sections = [
        ...
        ("5. Activity Diagram: Search for Resources",
         ...
         "Fusion, re-ranked with Cohere Rerank 4, filtered by metadata and RBAC permissions, and displayed to the user. If "
        ...
        ("6. Sequence Diagram: Search for Resources",
         ...
         "API, NLP engine, unified PostgreSQL store (pgvector + ParadeDB), reranker, and "
         "ranking service.",
         "sequence", 480,
         ...
         "Fusion and then re-ranks them with Cohere Rerank 4, applying any metadata filters. "
         "The ranked results are returned to the frontend for display. The query is logged in "
         "PostgreSQL for analytics and audit purposes."),
        ("7. Component Diagram",
         ...
         "Reranker (Cohere Rerank 4 for re-ranking merged results), and the Ranking Service "
         "(Reciprocal Rank Fusion of keyword and semantic results). The Data Layer includes a "
         "unified PostgreSQL database with pgvector for semantic search over document embeddings "
         "and ParadeDB for BM25 keyword search, structured metadata, and audit logs. The "
         "Document Ingestion Pipeline uses Docling to parse incoming documents, extracts text "
         "and metadata, chunks content, generates embeddings, and stores everything in "
         "PostgreSQL with cross-referenced document IDs."),
        ("8. Deployment Diagram",
         ...
         "low-latency inference; (3) the Database Container runs PostgreSQL 16 with the pgvector "
         "extension for semantic search over document embeddings and ParadeDB for BM25 keyword "
         "search, storing all document metadata, vectors, and audit logs in a single unified "
         "store; (4) the Keycloak Container provides identity management and SSO, integrated "
         "with NextAuth.js on the frontend for authentication. Communication between the "
         "frontend and backend uses REST API calls. The backend communicates with PostgreSQL "
         "via SQL/ORM for all data and search operations."),
    ]
